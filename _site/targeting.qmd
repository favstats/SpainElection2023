...
election_title <- "2023 Spanish election"
the_country <- "Spain"
...
This section shows spening per *targeting criteria* used by political parties to reach citizens in the run-up to the `r election_title` for the latest available data.
...
get_contested_graph <- function(ppp, minamount = 50, n_parties = 3) {
...
thegraph30 <- get_contested_graph(election_dat30, minamount = 200)
...
thegraph7 <- get_contested_graph(election_dat7, minamount = 200)
...
region_dat <- election_dat30 %>%
  ...
  ungroup() %>%
  rename(region = value) %>%
  mutate(region = str_remove_all(region, ", Spain")) 
